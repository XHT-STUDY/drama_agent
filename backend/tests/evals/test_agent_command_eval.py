...
class TestCommandDatasetContract:
    def test_dataset_version_and_floor(self) -> None:
        data = load_dataset()
        assert data["version"] == DATASET_VERSION
        assert len(data["cases"]) >= 85  # 65 重标注 + ≥20 continue 基础集
    ...
    def test_field_dependencies_are_legal(self) -> None:
        for c in load_cases():
            ...
            if exp["episode_number"] is not None:
                ...
                # revise_script / explain 的集数目标必须是 script
                assert exp["intent"] in {"evaluate", "revise_script", "explain"}
                if exp["intent"] != "evaluate":
...
class TestRealModelCommandEval:
    def test_run_full_dataset_and_report_metrics(self) -> None:
        """全量数据集 → 联合指标 + §3.3 门槛 → 结果落盘 results/。

        默认发版模式：任一门槛 fail 即测试失败（非零退出）。
        EVAL_REPORT_ONLY=1：开发期只产报告，不设门槛。
        """
        ...
        for case in cases:
            try:
                output = cast(
                    AgentPlannerOutput,
                    asyncio_run(
                        skill.execute({"input": planner_input(case), "agent": agent, "prompt_loader": loader})
                    ),
                )
        ...
        report: dict[str, Any] = {
            ...
            "dataset_version": data["version"],
            "dataset_split_counts": data.get("splits"),
            ...
            "gate_summary": {
                ...
                "no_data": [g["gate"] for g in gates if g["status"] == "no_data"],
            },
            "failures": [
                {
                    "id": s.case_id,
        ...
        assert metrics["parse_rate"] is not None and metrics["parse_rate"] > 0.8, (
            "超过 20% 用例调用/解析失败，结果不可用"
        )
        if not REPORT_ONLY:
            # 发版模式：fail 与 no_data 都阻断（§6.4 任一关键门槛不达标即非零退出）
            blocking = release_blocking_gates(gates)
            assert not blocking, "发版门槛未达标:\n" + "\n".join(
                f"  {g['gate']}: value={g['value']} threshold={g['threshold']}"
