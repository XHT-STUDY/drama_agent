...
> 文档版本：v1.6
> 编制日期：2026-08-16  
> 项目阶段：MVP — Phase A~I 全部完成，发布候选 **v0.1.0-rc1**；Phase J Agent 化增强**全部完成**（J-01～J-12；M1～M4 里程碑达成）；Phase D（RAG 骨架）已完成；v0.2.0 已定版（K-0 DONE）；Phase K 进行中（K-1、K-2 DONE；K-3～K-4 待启动，§20.5）
...
| J-09 | Action 生命周期、Outcome 与再规划 | 2d | J-04,J-05,J-06,J-08 | DONE | AI Agent | 2026-08-23：AgentActionLifecycle（mark_running + finalize 幂等回写：queued→running→completed/needs_review/failed/cancelled；崩溃后 GET Action/后台 reconcile 从 state_summary 重放）；AgentOutcomeService 确定性证据优先（failed→blocked、needs_review→partial、revise_outline 依赖剧本→partial+follow_ups、score_delta 从评估报告计算；语义约束才调 AgentOutcomeEvaluatorSkill，模型输出 Schema 无 goal_status/score/evidence 字段无从越权）；一次后续计划：partial/blocked+depth 0+白名单建议→服务端用最新 Artifact 重解析目标建 proposed 子 Action（follow-up Turn 以 result 消息为触发，(parent,depth) 唯一+SAVEPOINT 幂等，depth 1 不再生成子提案）；结果/计划消息按 (conversation,kind,agent_action_id,message_type) 幂等；EventPublisher.publish_agent_action_event（agent_action.updated 携带 agent_action_id/goal_status，旧消费者兼容）；Dispatcher 接线（启动 mark_running、终态/失败/取消 finalize）；confirm config 携带 agent_action_id；计划模板抽为模块函数父子共用；5 单测+3 skill 单测+3 事件集成+2 API anchor；全量 1083 passed/6 deselected，Ruff/mypy clean（318 files） |
| J-10 | 前端 API 与 Hooks | 1d | J-04,J-09 | DONE | AI Agent | 2026-08-23：types/api.ts 补 Conversation/ChatMessage/AgentTurn(TurnStatus/TurnType)/AgentAction(Status/Plan/Command 判别联合)/AgentOutcome/ActiveArtifactContext/ArtifactSnapshot/AgentRunSnapshot/AgentConfirmResponse（与 Pydantic extra=forbid 对齐）；api-client 新增 agentApi（createTurn 返回 {status,data} 支持 202/getTurn/getAction/confirm/reject）与 conversationsApi（create/list/messages 分页）；use-agent-conversation（当前会话管理+首条消息自动建会话、消息分页拼接、发送幂等 key 失败重发复用、202 planning 轮询至终态、action_id 失效、lastFailedContent 保留输入）；use-agent-action（非终态轮询可配间隔/终态自动停/卸载即停、confirm in-flight 防重复点击、重复确认复用服务端 Run 不建重复本地状态、ACTION_STALE→isStale 可恢复错误保留计划、query invalidation）+useAgentActionEvents（SSE agent_action.updated，卸载断开）；tests/agent-api.test.ts 5 例（TDD anchor: confirming_same_action_twice_reuses_run + 幂等 key 复用 + 202 轮询 + stale + 卸载停止）；前端全量 174 passed（169→174），tsc/lint clean |
| J-11 | Agent Workspace UI | 2d | J-09,J-10 | DONE | AI Agent | 2026-08-23：六组件（AgentWorkspace 双栏 7:5/8:4·平板抽屉·移动单栏 sticky Composer；ConversationPanel 会话选择+新建；MessageList 按 kind 渲染——澄清卡无确认按钮/action_result 展示 goal_status+评分变化+剩余约束+证据链接/action_plan 经插槽渲染计划卡；AgentComposer 迁移 ChatInput 首创能力保留集数设置·Enter 发送 Shift+Enter 换行·失败恢复草稿·aria-live·4 命令示例；ActionPlanCard 目标/来源版本/约束/步骤/影响+确认唯一主按钮防重复+stale/needs_review/failed 恢复入口；ArtifactContextPanel 最新 SB/大纲/各集剧本索引+active context 设置/清除）；globals.css 设计令牌（surface/text/border/accent/状态色 CSS variables·150/250ms·reduced motion·44px·focus-visible）+layout max-w-7xl；项目页 NEXT_PUBLIC_AGENT_WORKSPACE_ENABLED 开关（false 恢复 ChatInput+RunProgress，两模式共用 API/Artifact）；右栏计划卡与消息流去重；7 个组件测试（TDD anchor: ambiguous_turn_renders_clarification_without_confirmation_button + 示例发送/计划确认/结果渲染/键盘/草稿恢复/回滚开关）；前端全量 181 passed（174→181），tsc/lint clean |
| J-12 | E2E、评测与文档 | 2.5d | J-01..J-11 | DONE | AI Agent | 2026-08-23：e2e/agent-workspace.spec.ts 6 用例（串行共享项目；TDD anchors: script_revision_from_chat_produces_version_diff、partial_outcome_proposes_one_confirmable_follow_up；覆盖首次创作计划→确认→完成+刷新恢复、澄清无 Run、重复发送不重复消息、大纲修订受影响警告、重复确认单 Run）；FAKE_LLM_SCENARIO=agent_e2e（内容感知 planner 桩——从注入边界提取用户原文路由五意图+explain；低分评估+outline_reviser fixtures；e2e.sh 补 checkpoints setup）；tests/evals agent_commands.json 55 条（五意图/指代/模糊剧集/active context/冲突/越界）+ agent_outcomes.json 32 条（三态/证据/语义约束/非法建议/深度上限）+ 双 harness（CI：preflight 澄清召回 100% 零模型调用、Outcome 确定性规则 100%；eval_real marker 默认排除，真实评测落盘 results/ 不写模拟数字）；RunResponse 增 agent_action_id；docs/AGENT_EVAL_REPORT.md + TEST_PLAN §11 + API_CONTRACT；门禁：make lint/typecheck/test（后端 1098 passed/8 deselected、前端 181 passed）与 make e2e REPEAT=5 见 DEV_LOG |
