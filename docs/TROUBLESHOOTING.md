...
## 2026-09-13 — Planner 上下文二次截断切掉受保护目标（W1-03 修复）

**症状**：Planner 的上下文组装已声明"user_request 与 current_target 受保护、超预算即抛错"，但部分请求仍出现 Planner 拿到的上下文里目标信息缺失，模型对"改这一稿"类请求目标判断漂移。

**产生原因**：`AgentCommandService` 在 ContextBuilder 产出后又做了一次 `context_text[:12000]` 字符级截断。Builder 的预算单位是 token（12000 token ≈ 数万字符），其产出的组合文本字符数可远超 12000；第二次按字符切尾时，排在组装顺序后部的受保护段（current_target）被无差别切掉——两层裁剪的单位不同，后一层不知道前一层已保证的内容边界。

**解决方案**：删除第二次字符截断，`AgentPlannerInput.project_context` 上限放宽到 60000（仅防异常超大输入）。预算裁剪只存在一层：ContextBuilder 按 token 预算分配并对受保护段"无法完整保留即抛 ContextTooLargeError"。

**学习收获**：同一份数据只能有一个裁剪权威——在预算系统之外再做一层"保险式截断"，等于绕过预算系统对受保护内容的承诺；单位不同的两层裁剪尤其危险（第二层总是比第一层切得更深）。---
