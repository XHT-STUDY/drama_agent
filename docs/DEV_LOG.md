...
## L-5 分阶段默认体验与 Planner v1.1（2026-08-24，用户实测反馈修复）

### 做了什么

**用户实测暴露的问题**："我想写一个足球少年逆袭故事" → Planner 澄清"先设定大纲还是直接写剧本"；答"先搭建故事设定和大纲" → 又澄清"不支持新建设定大纲，要改 explain 吗"。两轮连环澄清，入口体验断裂。

- **Planner prompt v1.1**（模板 + manifest 同步）：规则 8——典型创作请求（我想写/帮我写/来一个 XX 故事）直接判 `create_script` 不澄清（愿望式表达=发起创作，集数有默认）；规则 9——分阶段是默认流程，"先搭建设定和大纲""先出大纲我看看"同样判 `create_script`，不是新意图不澄清；附 4 条输入→输出示例。评测集补 5 条用例（愿望式 ×3、分阶段表达 ×2），共 60 条。
- **staged 默认 true**（Agent Turn 路径）：分阶段成为默认体验——入口不再需要开关；"一口气写完" = 大纲门上点"写全部"（多一次确认，符合产品理念）；`staged=false` 保留为显式直通；legacy 直连 `POST /runs`（action=create_script）行为完全不变。前端 Composer 移除分阶段开关（保留集数设置），类型/Hook 清理。
- **门 UI 与进度改由 Run 驱动**：门后 Action 停在 needs_review 终态不再变化（J-01 状态机），此前计划卡按钮与 workspace RunProgress 都依赖 Action 状态 → 续跑后 UI 不跟随。改为共享 `gated-run` Run 查询（refetchInterval 随 queued/running 轮询）：续跑后门按钮消失（stage_gate 已剥离）、下一批门自动复现、RunProgress 随 Run 状态出现。
- E2E 全链路适配默认分阶段：确认计划 → 大纲门 → "写全部" → 完成；断言"续跑启动"用门按钮隐藏（FakeLLM 执行窗口 <1s，瞬时进度不可断言）。

### 为什么这么做

- 两个澄清轮次根因不同：第一轮是 Planner 对愿望式表达保守（prompt 缺引导）；第二轮是"分阶段"能力存在于请求参数而非对话语义——默认化让对话路径与产品行为合一，缺口自然消解。
- 门 UI 切换到 Run 驱动是修复而非优化：Action 状态机把 needs_review 设计为终态（J-01 审计语义），门后 Action 永远不变，只有 Run 能表达"已续跑/下一门"。

### 验证结果

| 命令 | 结果 |
| --- | --- |
| uv run pytest --disable-warnings | **1122 passed / 8 deselected** |
| uv run pytest tests/evals | CI 契约评测全绿（60 条数据集契约通过） |
| cd frontend && pnpm test | **190 passed**（移除 2 个开关用例） |
| bash scripts/e2e.sh --repeat-each=1 | **8 passed**（默认分阶段全链路） |
| Ruff / mypy / ESLint / tsc | 全部通过 |

### 学到了什么

1. FakeLLM 的执行速度快到 UI 轮询永远错过"执行中"窗口——E2E 断言要选状态迁移的确定信号（按钮消失/终态文本），不要断言瞬态。
2. 状态机终态（needs_review Action）承担审计语义后，前端一切"进行中"表达必须换数据源（Run），否则 UI 冻结在门上。
